...
-->
<!--
  Updated for ORFEUS SiteXML

  Info: Kiriaki Konstantinidou (kiriaki@itsak.gr)
-->
<!-- 
  File:
     xs3p.xsl
...
<xsl:stylesheet 
    ...
    <xsl:param name="title">SERA SiteXML Documentation</xsl:param>
    ...
    <!-- If 'true', prints all sub-types in the
            ...
            type hierarchy box. -->
    <xsl:param name="printAllSubTypes">false</xsl:param>
    ...
    <xsl:param name="externalCSSURL">./siteXML.css</xsl:param>
    ...
    */
    ...
    <xsl:template match="xsd:appinfo | xsd:documentation" mode="properties">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <!-- Replaced to preserve HTML Markup
    ...
    <xsl:template match="xsd:complexType" mode="properties">
        ...
        <div class="doc-block">
         ...
         <!-- ************************************************************ -->
        <!-- 🆕 NEW SECTION: ATTRIBUTE LIST                                -->
        <!-- ************************************************************ -->
        <xsl:if test=".//xsd:attribute | .//xsd:attributeGroup">
            <h3>Attributes of <xsl:text>&lt;</xsl:text>
                <xsl:value-of select="@name"/>
                <xsl:text>&gt;</xsl:text></h3>
            <table class="element-properties">
                <tr>
                <th width="20%">Name</th>
                <th width="20%">Type</th>
                <th width="15%">Use</th>
                ...
                <tr>
                    ...
                    <td>
                        <xsl:apply-templates select="xsd:annotation/xsd:documentation" mode="properties"/>
                    </td>
                </tr>
                </xsl:for-each>
                <!-- Attributes coming from groups -->
                <xsl:for-each select=".//xsd:attributeGroup">
                <tr>
                    <td colspan="5">
                        From attributeGroup: <b><xsl:value-of select="@ref"/></b>
                    </td>
                </tr>
                </xsl:for-each>
            </table>
        ...
        <!-- ************************************************************ -->
        <!-- 🆕 NEW SECTION: ELEMENT LIST (CONTENT MODEL)                  -->
        <!-- ************************************************************ -->
        <xsl:if test=".//xsd:sequence | .//xsd:choice | .//xsd:all | .//xsd:element">
            <h3>Elements of <xsl:text>&lt;</xsl:text>
                <xsl:value-of select="@name"/>
                <xsl:text>&gt;</xsl:text></h3>
            <table class="element-properties">
                <tr>
                ...
                <!-- All child elements inside the complexType -->
                <tr>
                    <td><xsl:value-of select="@name"/></td>
                    <!-- Print element type with reference -->
                    <td>
                        <xsl:choose>
                            <xsl:when test="xsd:simpleType">
                                <xsl:text>Locally-defined simple type</xsl:text>
                    ...
                    <td>
                        ...
                        <xsl:if test="number(@maxOccurs) > 1 or @maxOccurs = 'unbounded'">
                            , many
                        </xsl:if>
                    </td>
                    <td>
